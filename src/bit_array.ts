...
module bit_array {
  ...
  export class BitArray implements IndexKey {
    words: Array<number>;
    idx: number;
    offset: number;
    readOffset: number;
    ...
    packArray(array: Array) {
      array.forEach(el => {
        this.pack(el);
        if (util.typeOf(el) === util.ObjectType.String) { 
    ...
    pack(obj) {
      ...
      if (obj === null) {
        ...
        if ((type = util.typeOf(obj)) === util.ObjectType.Boolean) {
          if (obj.valueOf()) this.write(2, 4);
          else this.write(3, 4);
        } else if (type === util.ObjectType.Number) {
          this.write(4, 4);
          this.packNumber(obj);
        } else if (type === util.ObjectType.String) {
          this.write(5, 4);
          this.packString(obj);
        } else if (type === util.ObjectType.Uid) {
          this.write(6, 4);
          this.packUid(obj);
        } else if (type === util.ObjectType.Array) {
          this.write(15, 4);
          this.packArray(obj);
        } else {
