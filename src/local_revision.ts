...
module local_revision {
  export class LocalRevision extends util.Emitter implements Transaction {
    id: util.Uid;
    ...
    constructor(db: local_database.LocalDatabase, dbStorage: DbStorage, masterRef: ObjectRef,
        suffix: string) {
      super();
      var errorCb = (err: Error) => this.emit('error', err);
      ...
      this.uidGenerator = new util.UidGenerator(suffix);
      this.id = this.uidGenerator.generate();
      this.queue = new util.JobQueue();
      this.queue.on('error', errorCb);
      this.treeCache = {};
      this.master = new avl.AvlTree(dbStorage, masterRef, 0);
  ...
  class IndexProxy implements IndexTree {
    ...
    constructor(name: string, master: IndexTree, dbStorage: DbStorage,
      ...
      var getCb = (err: Error, refCount: Array) => {
        if (err) return cb(err);
        if (refCount) {
          this.tree = new avl.AvlTree(dbStorage, refCount[0], refCount[1]);
        } else {
          this.tree = new avl.AvlTree(dbStorage, null, null);
        }
        if (this.pending) {
    ...
    private delegate(cb: AnyCb, fn: AnyCb) {
      ...
      if (!this.pending) this.pending = new util.JobQueue(true);
