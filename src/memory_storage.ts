...
module memory_storage {
  export class MemoryStorage implements DbStorage {
    ...
    get(key: string, cb: ObjectCb) {
      cb(null, util.denormalize(this.kv[key]));
    }

    set(key: string, obj: any, cb: DoneCb) {
      this.kv[key] = util.normalize(obj);
      cb(null);
    }
    ...
    getIndexNode(ref: ObjectRef, cb: ObjectCb) {
      cb(null, util.denormalize(this.indexNode[ref.valueOf()]));
    }
    ...
    getIndexData(ref: ObjectRef, cb: ObjectCb) {
      cb(null, util.denormalize(this.indexData[ref.valueOf()]));
    }
    ...
    private save(hash: any, obj: any, cb: RefCb) {
      ...
      hash[ref.valueOf()] = util.normalize(obj);
      cb(null, ref);
    }
  }
